...
module.exports.getById = (appContext) => {
    return async (req, res, next) => {
        try {
            const productId = Utilities.parseInt(req.params.id, 0);

            const models = appContext.getDB;
            const repository = new Repository(models);
            const service = new Service(repository);

            const data = await service.getById(productId);
...
module.exports.update = (appContext) => {
    return async (req, res, next) => {
        try {
            ...
            const data = await service.update(productId, body);
...
module.exports.delete = (appContext) => {
    return async (req, res, next) => {
        try {
            ...
            const data = await service.delete(productId);
